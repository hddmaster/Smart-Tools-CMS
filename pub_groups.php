...
if (isset($_POST['head']))
 {
   ...
   if(isset($_FILES['picture']['name']) &&
    ...
    //Добавляем...
    $query = "insert into publications (parent_id, type, date, head, img_path, user_id)
                                       values
   ...
   if ($action == 'del')
    {
      if ($user->check_user_rules('delete'))
       {
      $result = @mysql_query("select * from publications where parent_id=$pub_id");
           ...
           if($row['img_path'])
             {
               ...
               if(!use_file($filename,'publications','img_path')) @unlink($_SERVER['DOCUMENT_ROOT'].$filename);
             }
...
function show_users($parent_id)
 {
   ...
   $select_users = '<select name="user_id" style="width:280px;" size="9">';
 ...
 function show_select($parent_id = 0, $prefix = '')
  {
    ...
    if(@mysql_num_rows($result) > 0)
    {
      while ($row = mysql_fetch_array($result))
        {
          $options .= '<option value="'.$row['pub_id'].'">'.$prefix.htmlspecialchars($row['head']).'</option>'."\n";
...
if (isset($_GET['parent_id']) && trim($_GET['parent_id']) !== '')
 ...
 $query = "select
	   ...
	   auth_site.username
           from publications left join auth_site on publications.user_id = auth_site.user_id
           where publications.type = 1 $add";
 ...
 navigation($page, $per_page, $total_rows, $params);
      ...
      echo '<tr align="center" class="header">
              <td nowrap width="50">№&nbsp;&nbsp;
                <a href="'.$_SERVER['PHP_SELF'].'?sort_by=pub_id&order=asc&page='.($page+1).'&per_page='.$per_page.'"><img src="/admin/images/'; if ($sort_by == 'pub_id' && $order == 'asc') echo 'sort_asc_sel.gif'; else echo 'sort_asc.gif'; echo '" border="0" alt="Сортировка по возрастанию"></a>
                <a href="'.$_SERVER['PHP_SELF'].'?sort_by=pub_id&order=desc&page='.($page+1).'&per_page='.$per_page.'"><img src="/admin/images/'; if ($sort_by == 'pub_id' && $order == 'desc') echo 'sort_desc_sel.gif'; else echo 'sort_desc.gif'; echo '" border="0" alt="Сортировка по убыванию"></a></td>
              <td nowrap>Дата&nbsp;&nbsp;
