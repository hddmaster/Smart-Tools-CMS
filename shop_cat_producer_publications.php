...
if (isset($_POST['head']))
 {
   ...
   {
   if (trim($_POST['producer_id'])=='') {Header("Location: ".$_SERVER['PHP_SELF']."?message=formvalues"); exit();}
   ...
   if(isset($_FILES['picture']['name']) &&
    ...
    //Добавляем...
    $query = "insert into shop_cat_producer_publications values
              (null,$producer_id,'$date','$head','',''";
              if(isset($_FILES['picture']['name']) &&
                 is_uploaded_file($_FILES['picture']['tmp_name']))
   ...
   if ($action == 'del')
    {
      if ($user->check_user_rules('delete'))
       {
         //удаляем из бд
         mysql_query("delete from shop_cat_producer_publications where pub_id=$pub_id");
         //Обновление кэша связанных модулей на сайте
         $cache = new Cache; $cache->clear_cache_by_module();
   ...
   if ($action == 'activate')
    {
      if ($user->check_user_rules('action'))
       {
         mysql_query("update shop_cat_producer_publications set status=1 where pub_id=$pub_id") or die(mysql_error());
         //Обновление кэша связанных модулей на сайте
         $cache = new Cache; $cache->clear_cache_by_module();
       }
   ...
   if ($action == 'reserve')
    {
      if ($user->check_user_rules('action'))
       {
         mysql_query("update shop_cat_producer_publications set status=0 where pub_id=$pub_id") or die(mysql_error());
         //Обновление кэша связанных модулей на сайте
         $cache = new Cache; $cache->clear_cache_by_module();
       }
      else $user->no_rules('action');
...
// сортировка
 ...
 if (isset($_GET['query_str']) && trim($_GET['query_str']) !== '')
 {
   ...
   $add .= " and (pub_id like '$query_str' or
		  ...
		  text like '$query_str' or
                  text_text like '$query_str')";
 }

 $query = "select
           shop_cat_producer_publications.*,
	   date_format(shop_cat_producer_publications.date, '%d.%m.%Y (%H:%i:%s)') as date2,
	   shop_cat_producers.producer_name,
	   shop_cat_producers.producer_picture
           from shop_cat_producer_publications left join shop_cat_producers on shop_cat_producer_publications.producer_id = shop_cat_producers.producer_id
           where 1 $add";
 ...
 echo '<tr align="center" class="header">
         ...
         <td width="35">&nbsp;</td>
         <td nowrap>Назвавание производителя</td>
         <td width="35">&nbsp;</td>
         <td width="120">&nbsp;</td>
       </tr>';
 ...
 while ($row = mysql_fetch_array($result))
  {
   echo '<tr valign="center" onmouseover="this.style.backgroundColor='; echo "'#EFEFEF'"; echo';" onmouseout="this.style.backgroundColor='; echo "'white'"; echo ';" class="underline">
           ...
           <td>'; if ($row['producer_name']) echo htmlspecialchars($row['producer_name']); else echo '&nbsp;'; echo '</td>
           <td align="center">'; if ($row['producer_picture']) echo '<img align="absmiddle" src="/admin/images/img_resize.php?size=30&image='.rawurlencode($row['producer_picture']).'" border="0">'; else echo '&nbsp;'; echo '</td>
           <td nowrap align="center">
           <a href="javascript:sw(\'/admin/editors/edit_shop_cat_producer_pub_text.php?id='.$row['pub_id'].'&mode=full\');"><img align="absmiddle" src="/admin/images/icons/edit.png" border="0" alt="Редактировать текст"></a>
           &nbsp;<a href="javascript:sw(\'/admin/editors/edit_shop_cat_producer_pub.php?id='.$row['pub_id'].'\');"><img align="absmiddle" src="/admin/images/icons/pencil.png" border="0" alt="Редактировать"></a>
           &nbsp;';
