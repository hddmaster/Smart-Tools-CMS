...
if (isset($_POST['username']))
 {
  ...
  if (trim($_POST['username'])=='' || trim($_POST['parent_id'])=='') {Header("Location: ".$_SERVER['PHP_SELF']."?message=formvalues"); exit();}

  $username = $_POST['username'];
  $parent_id = $_POST['parent_id'];

  //уникальная запись! Добавляем в каталог...
  $query = "insert into auth_site (parent_id,
                                   type,
			           username)
                                  values
			          ($parent_id,
  ...
  if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?message=db"); exit();}
   ...
   // перенумеровываем
   $result = mysql_query("select * from auth_site where parent_id = $parent_id order by order_id asc");
...
//-----------------------------------------------------------------------------
// AJAX
function save_user_structure($save_string)
{
  ...
  for($no=0; $no<count($items); $no++)
   {
     ...
     mysql_query("update auth_site set parent_id = ".$tokens[1].", order_id = ".$orders[$tokens[1]]." where user_id = ".$tokens[0]);
   }
...
$xajax->registerFunction("save_user_structure");
 ...
 function show_select($parent_id = 0, $prefix = '')
  {
    ...
    if(@mysql_num_rows($result) > 0)
    {
      while ($row = mysql_fetch_array($result))
        {
          $options .= '<option value="'.$row['user_id'].'">'.$prefix.htmlspecialchars($row['username']).'</option>'."\n";
          show_select($row['user_id'],$prefix.'&nbsp;&nbsp;&nbsp;');
        }
    }
    return $options;
  }
 ...
 function show_tree($parent_id = 0)
  {
    ...
    if(@mysql_num_rows($result) > 0)
    {
      while ($row = mysql_fetch_array($result))
        {
          if (is_begin($row['user_id'], $row['parent_id'])) echo '<ul>'."\n";
          echo '<li id="node'.$row['user_id'].'"';
          ...
          echo ' <span class="grey"> (id: '.$row['user_id'].')</span>';
          if ($row['type'] == 1) show_tree($row['user_id']);
          echo '</li>'."\n";
          if (is_end($row['user_id'], $row['parent_id'])) echo '</ul>'."\n";
        }
    }
  }

function is_begin($user_id, $parent_id)
 {
   $result = mysql_query("select * from auth_site where parent_id = $parent_id and type != 2 order by order_id asc");
   ...
   if ($num > 0)
    {
      ...
      while ($row = mysql_fetch_array($result))
       {
         if ($k == 1 && $row['user_id'] == $user_id) {return true; break;}
         $k++;
       }
    }
   return false;
...
function is_end($user_id, $parent_id)
 {
   $result = mysql_query("select * from auth_site where parent_id = $parent_id and type != 2 order by order_id asc");
   $num = mysql_num_rows($result);
   if ($num > 0)
    {
      ...
      while ($row = mysql_fetch_array($result))
       {
         if ($k == $num && $row['user_id'] == $user_id) {return true; break;}
         $k++;
       }
    }
   return false;
...
?>
	...
	function save_tree()
   {
     ...
     xajax_save_user_structure(saveString);
   }
