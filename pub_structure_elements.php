...
function save_pub_structure($save_string)
{
  ...
  for($no = 0; $no < count($items); $no++)
   {
     ...
     mysql_query("update publications set parent_id = ".$tokens[1].", order_id = ".$orders[$tokens[1]]." where pub_id = ".$tokens[0]);
   }
...
$xajax->registerFunction("save_pub_structure");
//------------------------------------------------------------------------------
...
if ($user->check_user_rules('view','/admin/pub_structure.php')) $tabs->add_tab('/admin/pub_structure.php', 'Структура', 1);
if ($user->check_user_rules('view','/admin/pub_comments.php')) $tabs->add_tab('/admin/pub_comments.php', 'Комментарии');
$tabs->show_tabs();

$tabs2 = new Tabs;
$tabs2->level = 1;
if ($user->check_user_rules('view','/admin/pub_structure.php')) $tabs2->add_tab('/admin/pub_structure.php', 'Группы');
if ($user->check_user_rules('view','/admin/pub_structure_elements.php')) $tabs2->add_tab('/admin/pub_structure_elements.php', 'Публикации');
$tabs2->show_tabs();
      ...
      while ($row = mysql_fetch_array($result))
        {
          ...
          if ($row['pub_id'] == $parent_id_selected) $options .= ' selected';
          $options .= '>'.$prefix.htmlspecialchars($row['head']).'</option>'."\n";
          show_select($row['pub_id'], $prefix.'&nbsp;&nbsp;&nbsp;', $parent_id_selected);
        }
    }
    return $options;
  }
 ...
 function show_tree($parent_id = 0)
  {
    ...
    if(@mysql_num_rows($result) > 0)
    {
      while ($row = mysql_fetch_array($result))
        {
          if (is_begin($row['pub_id'], $row['parent_id'])) echo '<ul>'."\n";
          echo '<li id="node'.$row['pub_id'].'"';
          ...
          echo ' <span class="grey"> (id: '.$row['pub_id'].')</span>';
          show_tree($row['pub_id']);
          echo '</li>'."\n";
          if (is_end($row['pub_id'], $row['parent_id'])) echo '</ul>'."\n";
        }
    }
  }
   ...
   if ($num > 0)
    {
      ...
      while ($row = mysql_fetch_array($result))
       {
         if ($k == 1 && $row['pub_id'] == $element_id) {return true; break;}
         $k++;
       }
    }
   return false;
   ...
   if ($num > 0)
    {
      ...
      while ($row = mysql_fetch_array($result))
       {
         if ($k == $num && $row['pub_id'] == $element_id) {return true; break;}
         $k++;
       }
    }
   return false;
 ...
 if (isset($_GET['parent_id']) && $_GET['parent_id'] !== '0')
  {
    ...
    $result = mysql_query("select * from publications where pub_id = $parent_id");
...
function save_tree()
 {
   ...
   xajax_save_pub_structure(saveString);
 }
