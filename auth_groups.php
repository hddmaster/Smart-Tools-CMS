 ...
 if ($user->check_user_rules('add'))
  {
   ...
   $result = mysql_query("insert into auth (parent_id, type, username) values ($parent_id, 1, '$username')");
...
if (isset($_GET['action']) && isset($_GET['id']))
 {
   ...
   if ($action == 'del')
    {
     if ($user->check_user_rules('delete'))
      ...
      //проверка на удаление корневого пользователя
      if ($user_id == 1) {Header("Location: ".$_SERVER['PHP_SELF']."?message=admin"); exit();}
   ...
   if ($action == 'activate')
    {
      if ($user->check_user_rules('action') && $user_id > 1) mysql_query("update auth set status=1 where user_id=$user_id");
      else $user->no_rules('action');
    }

   if ($action == 'reserve')
    {
      if ($user->check_user_rules('action') && $user_id > 1) mysql_query("update auth set status=0 where user_id=$user_id");
      else $user->no_rules('action');
    }
 }
...
// сортировка
 ...
 $query = "select auth.* from auth where type = 1 $add";
 ...
 while ($row = mysql_fetch_array($result))
  {
   echo '<tr valign="center" onmouseover="this.style.backgroundColor=\'#EFEFEF\'" onmouseout="this.style.backgroundColor=\'white\'" class="underline">
           ...
           echo '<a href="javascript:sw(\'/admin/editors/edit_auth_user.php?id='.$row['user_id'].'\');"><img align="absmiddle" src="/admin/images/icons/pencil.png" border="0" alt="Редактировать пользователя"></a>&nbsp;';
           if($row['status'] == 0) echo '<a href="?action=activate&id='.$row['user_id'].'"><img align="absmiddle" src="/admin/images/icons/light-bulb.png" border="0" alt="Активность пользователя"></a>';
           else echo '<a href="?action=reserve&id='.$row['user_id'].'"><img align="absmiddle" src="/admin/images/icons/light-bulb-off.png" border="0" alt="Активность пользователя"></a>';
