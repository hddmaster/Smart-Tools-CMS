...
if (isset($_POST['name']) &&
    isset($_POST['text']))
 {
  ...
  //Добавляем...
  $result = mysql_query("insert into gallery_comments values (null, $element_id, '$date', '$text', '$name', 0, 0)");
   ...
   if ($action == 'del')
    {
      if ($user->check_user_rules('delete'))
       {
         //удаляем из бд
         mysql_query("delete from gallery_comments where comment_id=$comment_id");
         //Обновление кэша связанных модулей на сайте
         $cache = new Cache; $cache->clear_cache_by_module();
   ...
   if ($action == 'activate')
    {
      if ($user->check_user_rules('action'))
       {
         mysql_query("update gallery_comments set status=1 where comment_id=$comment_id") or die(mysql_error());
         //Обновление кэша связанных модулей на сайте
         $cache = new Cache; $cache->clear_cache_by_module();
       }
   ...
   if ($action == 'reserve')
    {
      if ($user->check_user_rules('action'))
       {
         mysql_query("update gallery_comments set status=0 where comment_id=$comment_id") or die(mysql_error());
         //Обновление кэша связанных модулей на сайте
         $cache = new Cache; $cache->clear_cache_by_module();
       }
      else $user->no_rules('action');
...
function show_elements($parent_id)
{
  ...
  $result = mysql_query("select * from gallery where type = 0 and parent_id = $parent_id order by order_id asc");
...
// сортировка
 ...
 $query = "select
           gallery_comments.*,
	   date_format(gallery_comments.date, '%d.%m.%Y (%H:%i:%s)') as date2,
	   gallery.element_name,
	   gallery.img_path1,
	   gallery.parent_id
           from gallery_comments, gallery
           where gallery_comments.element_id = gallery.element_id $add";
 ...
 while ($row = mysql_fetch_array($result))
  {
   echo '<tr valign="center" onmouseover="this.style.backgroundColor='; echo "'#EFEFEF'"; echo';" onmouseout="this.style.backgroundColor='; echo "'white'"; echo ';" class="underline">
           ...
           <td>';
           if ($row['parent_id'] == 0) echo '---Корень галереи---';
           else
            {
              $res = mysql_query("select element_name from gallery where element_id = ".$row['parent_id']);
           ...
           <a href="javascript:sw(\'/admin/editors/edit_gallery_comment.php?id='.$row['comment_id'].'\');"><img align="absmiddle" src="/admin/images/icons/pencil.png" border="0" alt="Редактировать комментарий"></a>
           &nbsp;';
