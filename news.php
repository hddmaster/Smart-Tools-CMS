...
if (isset($_POST['head']))
 {
   ...
   if(isset($_FILES['picture']['name']) &&
    ...
    //Добавляем...
    $query = "insert into news (parent_id, date, date_begin, head, source, source_url, url, img_path, user_id)
                               values
                               ($parent_id, '$date', '$date_begin', '$head', '$source', '$source_url', '$url', '$img_path_db', $user_id)";
            ...
            if ($row['img_path'])
             {
               ...
               if (!use_file($filename,'news','img_path'))
 ...
 function show_select($parent_id = 0, $prefix = '', $parent_id_added)
  {
    ...
    if(@mysql_num_rows($result) > 0)
    {
      while ($row = mysql_fetch_array($result))
        {
          $options .= '<option value="'.$row['news_id'].'"';
          if ($parent_id_added == $row['news_id']) $options .= ' selected';
          $options .= '>'.$prefix.htmlspecialchars($row['head']).'</option>'."\n";
          show_select($row['news_id'],$prefix.'&nbsp;&nbsp;&nbsp;', $parent_id_added);
        }
    }
    return $options;
  }
 ...
 function show_select_filter($parent_id = 0, $prefix = '', $parent_id_element = '')
  {
    ...
    if(@mysql_num_rows($result) > 0)
    {
      while ($row = mysql_fetch_array($result))
        {
          $options .= '<option value="'.$row['news_id'].'"';
          if ($parent_id_element == $row['news_id']) $options .= ' selected';
          $options .= '>'.$prefix.htmlspecialchars($row['head']).'</option>'."\n";
          show_select_filter($row['news_id'],$prefix.'&nbsp;&nbsp;&nbsp;',$parent_id_element);
        }
    }
    return $options;
  }
 ...
 echo '<div class="dhtmlgoodies_question">
        <table cellspacing="0" cellpadding="4">
		 <tr>
		   ...
		   <td><h2 class="nomargins">Добавить новость</h2></td>
		 </tr>
		</table>   
	   </div>
...
if (isset($_GET['query_str']) && trim($_GET['query_str']) !== '')
 {
   ...
   $add .= " and (news.news_id like '$query_str' or
                  news.head like '$query_str' or
                  news.text like '$query_str' or
                  news.text_full like '$query_str')";
 }
...
if (isset($_GET['parent_id']) && trim($_GET['parent_id']) !== '')
 ...
 if (mysql_num_rows($result) > 0)
 {
    navigation($page, $per_page, $total_rows, $params);
      ...
      echo '<tr align="center" class="header">
              <td align="left" nowrap width="80"><input id="maincheck" type="checkbox" value="0" onclick="if($(\'#maincheck\').attr(\'checked\')) $(\'.cbx\').attr(\'checked\', true); else $(\'.cbx\').attr(\'checked\', false);"> №&nbsp;&nbsp;
                <a href="'.$_SERVER['PHP_SELF'].'?sort_by=news_id&order=asc&page='.($page+1).'&per_page='.$per_page.$add_params.'"><img src="/admin/images/'; if ($sort_by == 'news_id' && $order == 'asc') echo 'sort_asc_sel.gif'; else echo 'sort_asc.gif'; echo '" border="0" alt="Сортировка по возрастанию"></a>
                <a href="'.$_SERVER['PHP_SELF'].'?sort_by=news_id&order=desc&page='.($page+1).'&per_page='.$per_page.$add_params.'"><img src="/admin/images/'; if ($sort_by == 'news_id' && $order == 'desc') echo 'sort_desc_sel.gif'; else echo 'sort_desc.gif'; echo '" border="0" alt="Сортировка по убыванию"></a></td>
              <td nowrap>Дата&nbsp;&nbsp;
   ...
   echo '<tr valign="center" onmouseover="this.style.backgroundColor=\'#EFEFEF\'" onmouseout="this.style.backgroundColor=\'white\'" class="underline">
                 <td align="left" class="small" nowrap><input class="cbx" type="checkbox" name="id[]" value="'.$row['news_id'].'"> '.$row['news_id'].'</td>
                 <td align="center">'.$row['date2'].'</td>
           <td>';
           if ($row['parent_id'] == 0) echo '---Корень каталога---';
           else
            {
              $res = mysql_query("select * from news where news_id = ".$row['parent_id']);
           ...
           echo '</td>
                 ...
                 <td nowrap align="center"><a href="javascript:sw(\'/admin/editors/edit_news.php?id='.$row['news_id'].'\');"><img align="absmiddle" src="/admin/images/icons/pencil.png" border="0" alt="Редактировать"></a>
                 &nbsp;';
           if($row['status'] == 0) echo '<a href="?action=activate&id='.$row['news_id'].'&sort_by='.$sort_by.'&order='.$order.'&page='.($page+1).'&per_page='.$per_page.$add_params.'"><img align="absmiddle" src="/admin/images/icons/light-bulb-off.png" border="0" alt="Активность"></a>';
           else echo '<a href="?action=reserve&id='.$row['news_id'].'&sort_by='.$sort_by.'&order='.$order.'&page='.($page+1).'&per_page='.$per_page.$add_params.'"><img align="absmiddle" src="/admin/images/icons/light-bulb.png" border="0" alt="Активность"></a>'; echo '
                 &nbsp;<a href="javascript:if(confirm(\'Вы действительно хотите удалить?\')){location.href=\'?action=del&id='.$row['news_id'].'&sort_by='.$sort_by.'&order='.$order.'&page='.($page+1).'&per_page='.$per_page.$add_params.'\';}"><img align="absmiddle" src="/admin/images/icons/cross.png" border="0" alt="Удалить"></a></td>
               </tr>';
   }
  echo '</table>';
