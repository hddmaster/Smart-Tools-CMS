...
if ($user->check_user_rules('view','/admin/news_structure.php')) $tabs->add_tab('/admin/news_structure.php', 'Структура', 1);
      ...
      while ($row = mysql_fetch_array($result))
        {
          ...
          if ($row['news_id'] == $parent_id_selected) $options .= ' selected';
          $options .= '>'.$prefix.htmlspecialchars($row['head']).'</option>'."\n";
          show_select($row['news_id'], $prefix.'&nbsp;&nbsp;&nbsp;', $parent_id_selected);
        }
    }
    return $options;
  }
      ...
      while ($row = mysql_fetch_array($result))
        {
          ...
          echo ' <span class="grey"> (id: '.$row['news_id'].')</span>';
 ...
 if (isset($_GET['parent_id']) && $_GET['parent_id'] !== '0')
  {
    ...
    $result = mysql_query("select * from news where news_id = $parent_id");
