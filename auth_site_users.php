...
if (isset($_POST['user_type_name'])&&
 ...
 if ($user->check_user_rules('add'))
  {
   if (trim($_POST['user_type_name'])=='') {Header("Location: ".$_SERVER['PHP_SELF']."?message=formvalues"); exit();}
   $user_type_name = $_POST['user_type_name'];
   $user_type_descr = $_POST['user_type_descr'];

   // проверка а повторное название
   if (use_field($user_type_name,'auth_site_users','user_type_name')) {Header("Location: ".$_SERVER['PHP_SELF']."?message=duplicate");exit();}

   $result = mysql_query("insert into auth_site_users values (null, '$user_type_name', '$user_type_descr')");
   ...
   if ($action == 'del')
    {
     if ($user->check_user_rules('delete'))
      {
     $result = mysql_query("select * from auth_site where user_type=$user_type");
      ...
      // если нет пользователей такого типа, то его можно удалять...
        ...
        mysql_query("delete from auth_site_users where user_type=$user_type");
...
// сортировка
 ...
 $query = "select * from auth_site_users $add";
 ...
 while ($row = mysql_fetch_array($result))
  {
   echo '<tr valign="center" onmouseover="this.style.backgroundColor=\'#EFEFEF\'" onmouseout="this.style.backgroundColor=\'white\'" class="underline">
           ...
           <a href="javascript:sw(\'/admin/editors/edit_auth_site_user_type.php?id='.$row['user_type'].'\');"><img align="absmiddle" src="/admin/images/icons/pencil.png" border="0" alt="Редактировать тип"></a>
           &nbsp;<a href="';
