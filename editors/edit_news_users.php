...
if (isset($_GET['id']) && isset($_POST['action']) && $_POST['action'] == 'save') {
    if ($user->check_user_rules('edit')) {
        $news_id = (int)$_GET['id'];
        foreach($_POST['ids'] as $user_id) mysql_query("delete from news_auth_site where news_id = $news_id and user_id = $user_id");
        foreach($_POST['users'] as $user_id) mysql_query("insert into news_auth_site (news_id, user_id) values ($news_id, ".(int)$user_id.")");
...
if(isset($_GET['id'])) {
    if ($user->check_user_rules('view')) {
        ...
        if (isset($_GET['status'])) {
            $add .= " and (select count(*) from news_auth_site where news_id = $news_id and user_id = A.user_id) in (".implode(',', $_GET['status']).")";
        ...
        $query = "  select
                    A.*,
                    (select count(*) from news_auth_site where news_id = $news_id and user_id = A.user_id) as checked,
                    U.user_type_name
        ...
        $result = mysql_query($query." order by $sort_by $order LIMIT $start, $per_page") or die($query.'<br />'.mysql_error());
