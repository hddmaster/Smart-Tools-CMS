...
if (isset($_POST['script_descr']) &&
   ...
   isset($_GET['id']))
 {
  ...
  if (trim($_POST['script_path'])=='')
   ...
   $result = mysql_query("select * from auth_scripts where script_path='$script_path' and script_id != $script_id");
   if (mysql_num_rows($result) > 0) {Header("Location: ".$_SERVER['PHP_SELF']."?id={$_GET['id']}&message=duplicate"); exit();}

   $result = mysql_query("update auth_scripts set script_descr='$script_descr', script_path='$script_path', group_id=$group_id where script_id=$script_id");
...
if(isset($_GET['id']))
 ...
 if ($user->check_user_rules('view'))
  {
   $script_id = (int)$_GET['id'];
   $result = mysql_query("select * from auth_scripts where script_id=$script_id");
   if (!$result) exit();
   ...
   while ($r = mysql_fetch_array($res))
    {
      ...
      if ($group_id == $r['group_id']) echo ' selected';
      echo '>'.htmlspecialchars($r['group_name']).'</option>';
    }
 }
echo '</select>
      </td>
