...
if (isset($_POST['name']) &&
 ...
 if ($user->check_user_rules('edit'))
  ...
  $result = mysql_query("update video_comments set date='$date', name='$name', text='$text' where comment_id=$comment_id");
  if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$comment_id&message=db"); exit();}

  if (isset($_POST['element_id']))
   {
     $result = mysql_query("update video_comments set element_id = {$_POST['element_id']} where comment_id=$comment_id");
     if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$comment_id&message=db"); exit();}
   }
...
function is_end($element_id, $parent_id)
 {
   ...
   $result = mysql_query("select *,
                          ...
                          from video, video_comments
                          where video_comments.element_id = video.element_id and
                          video_comments.comment_id=$comment_id");
   if (!$result) exit();
