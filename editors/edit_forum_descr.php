   ...
   if (isset($_POST['FCKeditor1']))
    {
      ...
      $result = mysql_query("update forum set description='$description' where element_id=$element_id");
   ...
   if (isset($_POST['FCKeditor2']))
    {
      ...
      $result = mysql_query("update forum set description_full='$description_full' where element_id=$element_id");
      if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$element_id&mode=$mode&message=db"); exit();}
    }
...
$tabs = new Tabs;
if ($user->check_user_rules('view','/admin/editors/edit_forum_descr.php')) $tabs->add_tab('/admin/editors/edit_forum_descr.php?id='.$element_id.'&mode=brief', 'Краткое описание', ((preg_match('/brief/', $_SERVER['REQUEST_URI'])) ? 1 : 0));
if ($user->check_user_rules('view','/admin/editors/edit_forum_descr.php')) $tabs->add_tab('/admin/editors/edit_forum_descr.php?id='.$element_id.'&mode=full', 'Подробное описание', ((preg_match('/full/', $_SERVER['REQUEST_URI'])) ? 1 : 0));
$tabs->show_tabs();
