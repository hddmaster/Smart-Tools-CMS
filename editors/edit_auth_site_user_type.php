 ...
 if ($user->check_user_rules('edit'))
  {
   ...
   $result = mysql_query("select * from auth_site_users where user_type_name='$user_type_name'");
   while ($row = mysql_fetch_array($result))
    {
      if ($row['user_type'] !== $user_type) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$user_type&message=duplicate"); exit();}
    }
   $result = mysql_query("update auth_site_users set user_type_name='$user_type_name', user_type_descr='$user_type_descr' where user_type=$user_type");
...
if(isset($_GET['id']))
 ...
 if ($user->check_user_rules('view'))
  {
   $user_type = (int)$_GET['id'];
   $result = mysql_query("select * from auth_site_users where user_type=$user_type");
   if (!$result) exit();
