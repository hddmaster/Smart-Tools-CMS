   ...
   if (isset($_POST['FCKeditor1']))
    {
      ...
      $result = mysql_query("update shop_cat_producer_publications set text='$text' where pub_id=$pub_id");
   ...
   if (isset($_POST['FCKeditor2']))
    {
      ...
      $result = mysql_query("update shop_cat_producer_publications set text_full='$text_full' where pub_id=$pub_id");
      if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$pub_id&mode=$mode&message=db"); exit();}
    }
...
if(isset($_GET['id']))
 ...
 echo '<h2>'.htmlspecialchars($head).'</h2>';

$tabs = new Tabs;
if ($user->check_user_rules('view','/admin/editors/edit_shop_cat_producer_pub_text.php')) $tabs->add_tab('/admin/editors/edit_shop_cat_producer_pub_text.php?id='.$pub_id.'&mode=brief', 'Краткое описание', ((preg_match('/brief/', $_SERVER['REQUEST_URI'])) ? 1 : 0));
if ($user->check_user_rules('view','/admin/editors/edit_shop_cat_producer_pub_text.php')) $tabs->add_tab('/admin/editors/edit_shop_cat_producer_pub_text.php?id='.$pub_id.'&mode=full', 'Подробное описание', ((preg_match('/full/', $_SERVER['REQUEST_URI'])) ? 1 : 0));
$tabs->show_tabs();
