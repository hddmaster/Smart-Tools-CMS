...
$user_file_name = $name.'.jpg';
  ...
  $query = "insert into shop_cat_gallery values (null, $parent_id, $element_id, 0, 1, '$element_name'";
   ...
   if ($action == 'del')
    {
      if ($user->check_user_rules('delete'))
       {
            $result = mysql_query("select * from shop_cat_gallery where parent_id = $element_id");
            ...
            if($row['img_path'])
             {
               ...
               if(!use_file($filename,'shop_cat_gallery','img_path')) unlink($_SERVER['DOCUMENT_ROOT'].$filename);
             }
 ...
 while ($row = mysql_fetch_array($result))
  {
   ...
   echo '<tr valign="center" onmouseover="this.style.backgroundColor='; echo "'#EFEFEF'"; echo';" onmouseout="this.style.backgroundColor='; echo "'white'"; echo ';" class="underline">
           ...
           <td align="center">'; if ($row['img_path']) echo '<img align="absmiddle" src="/admin/images/img_resize.php?size=30&image='.rawurlencode($row['img_path']).'" alt="'.$row['img_path'].'" border="0">'; else echo '&nbsp;'; echo '</td>
