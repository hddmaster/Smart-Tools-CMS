 ...
 if ($user->check_user_rules('edit'))
  {
   ...
   $data = stripslashes($_POST['file_data']);
   $handle = @fopen($path, "w");
   if ($handle)
    {
   ...
   else Header("Location: ".$_SERVER['PHP_SELF']."?path=".urlencode($path_clear)."&message=norules");
   ...
   $rules = $_POST['rules'];
   if (!chmod($path,intval($rules,8))) Header("Location: ".$_SERVER['PHP_SELF']."?path=".urlencode($path_clear)."&message=no_rules");
 ...
 if ($user->check_user_rules('view'))
  {
   ...
   else Header("Location: ".$_SERVER['PHP_SELF']."?path=".urlencode($path_clear)."&message=no_rules");
...
echo '<form action="?path='.urlencode($path_clear).'" method="post">
   <table cellpadding="4" cellspacing="1" border="0" class="form">
    <tr>
      <td>Название файла <sup class="red">*</sup></td>
    ...
    <tr>
      ...
      <td><input style="width:30px" type="text" name="rules" value="'.$rules.'" maxlength="4" onKeyPress ="if (event.keyCode < 48 || event.keyCode > 57) event.returnValue = false;"></input></td></tr>
   </table><br>
      <textarea wrap="off" style="font-family:Courier New;font-size:10pt;width:100%;height:380px" name="file_data">'.htmlspecialchars($data).'</textarea>
