...
if (isset($_GET['id']) && isset($_POST['action']) && $_POST['action'] == 'save') {
    if ($user->check_user_rules('edit')) {
        $file_id = (int)$_GET['id'];
        foreach($_POST['ids'] as $user_id) mysql_query("delete from files_auth_site where file_id = $file_id and user_id = $user_id");
        foreach($_POST['users'] as $user_id) mysql_query("insert into files_auth_site (file_id, user_id) values ($file_id, ".(int)$user_id.")");
...
if(isset($_GET['id'])) {
    if ($user->check_user_rules('view')) {        
        ...
        if (isset($_GET['status'])) {
            $add .= " and (select count(*) from files_auth_site where file_id = $file_id and user_id = A.user_id) in (".implode(',', $_GET['status']).")";
        ...
        $query = "  select
                    A.*,
                    (select count(*) from files_auth_site where file_id = $file_id and user_id = A.user_id) as checked,
                    U.user_type_name
