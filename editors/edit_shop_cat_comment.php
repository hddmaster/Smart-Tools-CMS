...
if (isset($_POST['name']) &&
 ...
 if ($user->check_user_rules('edit'))
  ...
  $result = mysql_query("update shop_cat_comments set date='$date', name='$name', text='$text' where comment_id=$comment_id");
  if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$comment_id&message=db"); exit();}

  if (isset($_POST['element_id']))
   {
     $result = mysql_query("update shop_cat_comments set element_id = {$_POST['element_id']} where comment_id=$comment_id");
     if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$comment_id&message=db"); exit();}
   }
...
function show_select($parent_id = 0, $prefix = '', $selected_element_id = 0, &$shop_tree)
 {
   ...
   $result = mysql_query("select *,
                          ...
                          from shop_cat_elements, shop_cat_comments
                          where shop_cat_comments.element_id = shop_cat_elements.element_id and
                          shop_cat_comments.comment_id=$comment_id");
   if (!$result) exit();
