...
// picture3 end-----------------------------------------------------------------
  ...
  if (isset($_FILES['picture3']['name']) && is_uploaded_file($_FILES['picture3']['tmp_name']))
   ...
   $result = mysql_query("update forum set parent_id=$parent_id, element_name='$element_name', date='$date', order_id = $order_id where element_id=$element_id");
   if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$element_id&message=db"); exit();}
...
echo'      </td></tr>
    ...
    <tr>
      ...
      <td><select name="parent_id" style="width:280px;">
            ...
            <option value="0"'; if ($parent_id == 0) echo ' selected'; echo '>---Корень форума---</option>
