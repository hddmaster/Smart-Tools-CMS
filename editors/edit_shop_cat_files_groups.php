...
$name = $name_clear;
$i = 1;
 while (file_exists($_SERVER['DOCUMENT_ROOT']."/userfiles/shop_cat_files/$name.$ext"))
  {
   $name = $name_clear." ($i)";
   $i ++;
  }
$user_file_name = $name.'.jpg';
  ...
  $query = "insert into shop_cat_files values (null, $parent_id, $element_id, 0, 1, '$element_name'";
  if (isset($_FILES['file']['name']) && is_uploaded_file($_FILES['file']['tmp_name'])) $query .= ", '"."/userfiles/shop_cat_files/$user_file_name"."'";
  else $query .= ", ''";
  $query = $query.", $status)";
  ...
  if (isset($_FILES['file']['name']) && is_uploaded_file($_FILES['file']['tmp_name']))
   {
     $filename = $_SERVER['DOCUMENT_ROOT']."/userfiles/shop_cat_files/$user_file_name";
   ...
   if (@mysql_num_rows($result) > 0)
    {
      ...
      while ($row = mysql_fetch_array($result))
       {
         $id = $row['element_id'];
         mysql_query("update shop_cat_files set order_id=$i where element_id = $id");
         $i++;
       }
    }
 ...
 if ($user->check_user_rules('edit'))
  {
   ...
   foreach($_POST['element_names'] as $element_id => $element_name)
      mysql_query("update shop_cat_files set element_name = '$element_name' where element_id = $element_id");
   foreach($_POST['parent_ids'] as $element_id => $parent_id)
      mysql_query("update shop_cat_files set parent_id = $parent_id where element_id = $element_id");
...
if (isset($_GET['action']) && isset($_GET['id']))
 {
   ...
   $main_element_id = $_GET['element_id'];

   if ($action == 'del')
    {
      if ($user->check_user_rules('delete'))
       {
            $result = mysql_query("select * from shop_cat_files where parent_id = $element_id");
            if (mysql_num_rows($result) > 0)  {Header("Location: ".$_SERVER['PHP_SELF']."?id=$main_element_id&message=use"); exit();}
        
        
            $result = mysql_query("select * from shop_cat_files where element_id = $element_id");
            if (mysql_num_rows($result) > 0)
            {
            ...
            if($row['file_path'])
             {
               ...
               if(!use_file($filename,'shop_cat_files','file_path')) unlink($_SERVER['DOCUMENT_ROOT'].$filename);
             }
             
            //удаляем из files
            $result = mysql_query("delete from shop_cat_files where element_id=$element_id");
   ...
   if ($action == 'activate')
    {
      if ($user->check_user_rules('action'))
       {
         mysql_query("update shop_cat_files set status=1 where element_id=$element_id");
         //Обновление кэша связанных модулей на сайте
         $cache = new Cache; $cache->clear_cache_by_module();
       }
   ...
   if ($action == 'reserve')
    {
      if ($user->check_user_rules('action'))
       {
         mysql_query("update shop_cat_files set status=0 where element_id=$element_id");
         //Обновление кэша связанных модулей на сайте
         $cache = new Cache; $cache->clear_cache_by_module();
       }
      else $user->no_rules('action');
...
if ($user->check_user_rules('view','/admin/editors/edit_shop_cat_files_structure.php')) $tabs2->add_tab('/admin/editors/edit_shop_cat_files_structure.php?id='.$element_id, 'Структура');
$tabs2->show_tabs();
...
if (isset($_GET['message']))
 ...
 echo '<div class="dhtmlgoodies_question">
        <table cellspacing="0" cellpadding="4">
		 <tr>
		   ...
		   <td><h2 class="nomargins">Добавить новую группу</h2></td>
		 </tr>
		</table>   
	   </div>
 ...
 echo '<form enctype="multipart/form-data" action="?element_id='.$element_id.'" method="post">
   <table cellpadding="4" cellspacing="1" border="0" class="form">
    ...
    <tr>
      <td>Фотография</td>
      <td><input style="width:280px" type="file" name="file"/></td>
    </tr>
    <tr>
 ...
 while ($row = mysql_fetch_array($result))
  {
   ...
   echo '<tr valign="center" onmouseover="this.style.backgroundColor='; echo "'#EFEFEF'"; echo';" onmouseout="this.style.backgroundColor='; echo "'white'"; echo ';" class="underline">
           ...
           <td align="center">'; if ($row['file_path']) echo '<img align="absmiddle" src="/admin/images/img_resize.php?size=30&image='.rawurlencode($row['file_path']).'" alt="'.$row['file_path'].'" border="0">'; else echo '&nbsp;'; echo '</td>
           <td nowrap align="center">';
           if($row['status'] == 0) echo '<a href="?action=activate&element_id='.$element_id.'&id='.$row['element_id'].'&sort_by='.$sort_by.'&order='.$order.'&page='.($page+1).'&per_page='.$per_page.'"><img align="absmiddle" src="/admin/images/icons/light-bulb-off.png" border="0" alt="Активность"></a>';
           else echo '<a href="?action=reserve&element_id='.$element_id.'&id='.$row['element_id'].'&sort_by='.$sort_by.'&order='.$order.'&page='.($page+1).'&per_page='.$per_page.'"><img align="absmiddle" src="/admin/images/icons/light-bulb.png" border="0" alt="Активность"></a>'; echo '
           &nbsp;<a href="';
           echo "javascript:if(confirm('Вы действительно хотите удалить?')){location.href='?action=del&element_id=$element_id&id=".$row['element_id'].'&sort_by='.$sort_by.'&order='.$order.'&page='.($page+1).'&per_page='.$per_page."';}";
