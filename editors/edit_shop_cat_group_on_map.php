...
if (isset($_GET['delete_coodrinates']) && isset($_GET['id']))
 ...
 if ($user->check_user_rules('delete'))
  ...
  mysql_query("update shop_cat_elements set google_maps_latitude = '', google_maps_longitude = '' where element_id = $element_id") or die(mysql_error());
...
function save_position($element_id, $value)
{
  ...
  mysql_query("update shop_cat_elements set google_maps_latitude = $google_maps_latitude, google_maps_longitude = $google_maps_longitude where element_id = $element_id");
  ...
  $text = '<table cellspacing="0" cellpadding="2">
            <tr>
              ...
              <td><a href="javascript:if(confirm(\'Вы действительно хотите удалить?\')){location.href=\'edit_shop_cat_group_on_map.php?delete_coodrinates=true&id='.$element_id.'\';}"><img src="/admin/images/icons/cross.png" alt="Удалить" border="0"></a></td>
            </tr>
           </table>';
...
<?
$result = mysql_query("select * from shop_cat_elements 
                       where status = 1 and 
                       type = 1 and 
