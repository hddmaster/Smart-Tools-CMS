...
if (    isset($_POST['username']) &&
    ...
    if ($user->check_user_rules('edit')) {
        ...
        if ($password1 !== $password2) {
            Header("Location: ".$_SERVER['PHP_SELF']."?id=$user_id&message=passwords");
            exit();
        }

        $username = $_POST['username'];
        $result = mysql_query("select * from auth_site where username='$username' and user_id != $user_id and type = 0");
        ...
        if (trim($email) !== '' && !valid_email($email)) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$user_id&message=notvalidemail");exit();}
        ...
        foreach ($_POST['emails'] as $email) {
            if (trim($email)) {
                if (valid_email(trim($email))) $emails[] = strtolower(trim($email));
                else {
                    Header("Location: ".$_SERVER['PHP_SELF']."?id=$user_id&message=notvalidemail");
                    exit();
                }
            }
        }
        ...
        $res = mysql_query("show columns from auth_site");
        ...
        $values['register_date'] = substr($_POST['register_date'],6,4).substr($_POST['register_date'],3,2).substr($_POST['register_date'],0,2).$hour.$minute.$second;
        $values['user_birthday'] = substr($_POST['user_birthday'],6,4).substr($_POST['user_birthday'],3,2).substr($_POST['user_birthday'],0,2);
        unset($values['emails']);
        unset($values['password']);
        unset($values['user_image']);

        $result = mysql_query("select user_image from auth_site where user_id=$user_id");
  ...
  $user_file_name =  $name.'.'.$ext;
    ...
    $result = mysql_query("update auth_site set user_image='"."/userfiles/user_images/$user_file_name"."' where user_id=$user_id");
   ...
   if ($password1 !== '')
    {
      $result = mysql_query("update auth_site set password = '".md5($password1.SOLT)."' where user_id=$user_id");
      if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$user_id&message=db"); exit();}
    }
    
   $result = mysql_query("update auth_site set emails = '".(count($emails) > 0 ? serialize($emails) : '')."' where user_id=$user_id");
   if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$user_id&message=db"); exit();}

   mysql_query("delete from auth_site_user_properties where user_id = $user_id");
   foreach($_POST['properties'] as $property_id)
    {
      $result = mysql_query("insert into auth_site_user_properties values ($user_id, $property_id)");
      if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$user_id&message=db"); exit();}
    }
  ...
  if ($delete_img == 'true')
   {
     $result = mysql_query("select user_image from auth_site where user_id=$user_id");
     $row = mysql_fetch_array($result);
     if (!use_file($row['user_image'],'auth_site','user_image')) @unlink($_SERVER['DOCUMENT_ROOT'].$row['user_image']);
     $result = mysql_query("update auth_site set user_image='' where user_id=$user_id");
...
if(isset($_GET['id']))
 ...
 function show_select($parent_id = 0, $prefix = '', $parent_id_added, $excluded = array())
  {
    global $options;
      ...
      while ($row = mysql_fetch_array($result))
        {
          if(!in_array($row['user_id'], $excluded)) {
            ...
            show_select($row['user_id'],$prefix.'&nbsp;&nbsp;&nbsp;', $parent_id_added, $excluded);
          }
        }
    }
    return $options;
   ...
   $result = mysql_query("select
                          ...
                          U.user_type_name
                          from auth_site as A left join auth_site_users as U
...
if (isset($_GET['message']))
 ...
 echo '<form enctype="multipart/form-data" action="?id='.$user_id.'" method="post">
    ...
    <tr>
        ...
        <td>
            ...
            <table cellspacing="0" cellpadding="0">
                <tr>
                <td><input type="radio" name="user_sex" value="not selected"'.($row->user_sex == 'not selected' ? ' checked' : '').'></td>
                <td>&nbsp;Не указан</td>
                <td><img src="/admin/images/px.gif" alt="" width="10" height="1"></td>
                <td><input type="radio" name="user_sex" value="men"'.($row->user_sex == 'men' ? ' checked' : '').'></td>
                <td>&nbsp;Мужской</td>
                <td><img src="/admin/images/px.gif" alt="" width="10" height="1"></td>
                <td><input type="radio" name="user_sex" value="woman"'.($row->user_sex == 'woman' ? ' checked' : '').'></td>
                <td>&nbsp;Женский</td>
                </tr>
            </table>
...
echo'</td></tr>
   ...
   $oFCKeditor->Create() ;
    ...
    <tr>
      <td>e-mail</td>
      <td>
        <input style="width:280px" type="text" name="email" value="'.htmlspecialchars($row->email).'" maxlength="255">';
?>
<script>
function append_email() {
 var newItem=document.createElement('DIV');
 newItem.innerHTML='<input type="text" name="emails[]" style="width:280" value="">';
 document.getElementById("emails").appendChild(newItem);
}
</script>
<?
        echo '<div style="margin-top: 10px;">
          <div class="small" style="margin-bottom: 4px;">дополнительные адреса:</div>
          <div id="emails" style="margin-bottom: 4px;">';
           $emails = ($row->emails && count(unserialize($row->emails)) ? unserialize($row->emails) : array());
           if (count($emails) > 0) {
            foreach($emails as $email) echo '<div><input type="text" name="emails[]" value="'.$email.'" style="width: 280px;"></div>';
           } echo '<div><input type="text" name="emails[]" value="" style="width: 280px;"></div>';
          echo '</div>
          <button type="button" onclick="append_email()">добавить</button>
        </div>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td nowrap><input style="width:280px" name="ur_company_name" type="text" value="'.htmlspecialchars($row->ur_company_name).'"></td>
    </tr>
    <tr>
      <td>ИНН</td>
      <td nowrap><input style="width:280px" name="ur_inn" type="text" value="'.htmlspecialchars($row->ur_inn).'"></td>
    </tr>
    <tr>
      <td>КПП</td>
      <td nowrap><input style="width:280px" name="ur_kpp" type="text" value="'.htmlspecialchars($row->ur_kpp).'"></td>
    </tr>
    <tr>
      <td>Юридический адрес</td>
      <td nowrap><input style="width:280px" name="ur_address" type="text" value="'.htmlspecialchars($row->ur_address).'"></td>
    </tr>
    <tr>
      <td>Банк</td>
      <td nowrap><input style="width:280px" name="ur_bank" type="text" value="'.htmlspecialchars($row->ur_bank).'"></td>
    </tr>
    <tr>
      <td>Расчетный счет</td>
      <td nowrap><input style="width:280px" name="ur_rs" type="text" value="'.htmlspecialchars($row->ur_rs).'"></td>
    </tr>
    <tr>
      <td>Корреспондентский счет</td>
      <td nowrap><input style="width:280px" name="ur_ks" type="text" value="'.htmlspecialchars($row->ur_ks).'"></td>
    </tr>
    <tr>
      <td>БИК</td>
      <td nowrap><input style="width:280px" name="ur_bik" type="text" value="'.htmlspecialchars($row->ur_bik).'"></td>
    </tr>
    <tr>
 ...
 $res = mysql_query("select * from auth_site_users order by user_type_name asc");
...
echo'  </select>
    ...
    <tr>
      ...
      <td><select name="parent_id" style="width:280px;">
            ...
            '.show_select(0, '', $row->parent_id, array($row->user_id)).'
          </select>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      $res = mysql_query("select P.*, (select count(*) from auth_site_user_properties where user_id = $user_id and property_id = P.property_id) as c from auth_site_properties as P order by property_name asc");
      if (mysql_num_rows($res) > 0)
       {
         while ($r =  mysql_fetch_object($res))
          {
            echo '<div style="float: left;">
                   <table cellspacing="0" cellpadding="2">
                    <tr>
                     <td><input type="checkbox" name="properties[]" value="'.$r->property_id.'"'.(($r->c) ? ' checked' : '').'></td>
                     <td nowrap>'.htmlspecialchars($r->property_name).'</td>
                    </tr>
                   </table>
                  </div>';
          }
