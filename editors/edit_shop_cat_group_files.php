...
$user_file_name = $name.'.'.$ext;
  ...
  $query = "insert into shop_cat_files values (null, $parent_id, $element_id, 0, 0, '$element_name'";
...
define ('ELEMENT_ID', $element_id);
 ...
 function show_select($parent_id = 0, $prefix = '', $sel_id = 0)
  {
    global $options;
    $result = mysql_query("SELECT * FROM shop_cat_files where parent_id = $parent_id and type = 1 and catalog_element_id = ".ELEMENT_ID." order by order_id asc");
    if(@mysql_num_rows($result) > 0)
    {
      while ($row = mysql_fetch_array($result))
        {
 ...
 echo '<div class="dhtmlgoodies_question">
        <table cellspacing="0" cellpadding="4">
		 <tr>
		   ...
		   <td><h2 class="nomargins">Добавить новый файл</h2></td>
		 </tr>
		</table>   
	   </div>
 ...
 while ($row = mysql_fetch_array($result))
  {
   ...
   echo '<tr valign="center" onmouseover="this.style.backgroundColor='; echo "'#EFEFEF'"; echo';" onmouseout="this.style.backgroundColor='; echo "'white'"; echo ';" class="underline">
           ...
           <td align="center">'; if ($row['file_path']) echo '<a href="'.$row['file_path'].'" >'.str_replace('/userfiles/shop_cat_files/','',$row['file_path']).'</a>'; else echo '&nbsp;'; echo '</td>
