...
//если есть картинка, проверяем её тип
  ...
  if ($img_path != '')
   {
     if (!use_file($img_path,'news','img_path'))
  ...
  if (isset($_FILES['picture']['name']) && is_uploaded_file($_FILES['picture']['tmp_name']))
   {
    $result = mysql_query("update news set img_path='/userfiles/news_images/$user_file_name' where news_id=$news_id");
    $cache = new Cache;
    ...
    $result = mysql_query(" update
                            ...
                            where
                            news_id=$news_id");
    if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$news_id&message=db"); exit();}
...
if (isset($_GET['delete_img']) && isset($_GET['id']))
 ...
 if ($user->check_user_rules('delete'))
  {
  $news_id = (int)$_GET['id'];
  $result = mysql_query("select img_path from news where news_id=$news_id");
  $row = mysql_fetch_array($result);
  if (!use_file($row['img_path'],'news','img_path')) unlink($_SERVER['DOCUMENT_ROOT'].$row['img_path']);

  $result = mysql_query("update news set img_path='' where news_id=$news_id");
      ...
      while ($row = mysql_fetch_array($result))
        {
          ...
          show_select($row['news_id'],$prefix.'&nbsp;&nbsp;&nbsp;',$parent_id_element);
        }
    }
    return $options;
  }
 ...
 function show_select_users($parent_id = 0, $prefix = '', $group_id = 0)
  {
   ...
   $result = mysql_query("select
                          ...
                          date_format(N.date_begin, '%d.%m.%Y (%H:%i:%s)') as date_begin,
                          U.type as is_group,
                          U.parent_id as group_id
                          from
                          news as N left join auth_site as U
...
$tabs = new Tabs;
if ($user->check_user_rules('view','/admin/editors/edit_news.php')) $tabs->add_tab('/admin/editors/edit_news.php?id='.$news_id, 'Свойства');
if ($user->check_user_rules('view','/admin/editors/edit_news_text.php')) $tabs->add_tab('/admin/editors/edit_news_text.php?id='.$news_id.'&mode=brief', 'Краткое описание');
if ($user->check_user_rules('view','/admin/editors/edit_news_text.php')) $tabs->add_tab('/admin/editors/edit_news_text.php?id='.$news_id.'&mode=full', 'Подробное описание');
if ($user->check_user_rules('view','/admin/editors/edit_news_users.php')) $tabs->add_tab('/admin/editors/edit_news_users.php?id='.$news_id, 'Область видимости');
$tabs->show_tabs();
...
if (isset($_GET['message']))
 ...
 echo '<form enctype="multipart/form-data" action="?id='.$news_id.'" method="post">
   <table cellpadding="4" cellspacing="1" border="0" class="form">
    ...
    <tr>
      <td>Источник</td>
      <td><input style="width:280px" type="text" name="source" value="'.htmlspecialchars($row->source).'" maxlength="255"></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>URL источника</td>
      <td><input style="width:280px" type="text" name="source_url" value="'.htmlspecialchars($row->source_url).'" maxlength="255"></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>Фотография</td>
      <td><table cellspacing="0" cellpadding="0">
       ...
       if ($row->img_path)
        {
          echo '<a href="';
          echo "javascript:if(confirm('Вы действительно хотите удалить?')){location.href='?delete_img=1&id=$news_id';}";
