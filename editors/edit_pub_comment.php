 ...
 if ($user->check_user_rules('edit'))
  {
   ...
   if (trim($_POST['text'])=='' ||
    ...
    {Header("Location: ".$_SERVER['PHP_SELF']."?id=$comment_id&message=formvalues");exit();}
  ...
  $result = mysql_query("update pub_comments set date='$date', name='$name', text='$text' where comment_id=$comment_id");
  if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$comment_id&message=db"); exit();}

  if (isset($_POST['pub_id']))
   {
     $result = mysql_query("update pub_comments set pub_id = {$_POST['pub_id']} where comment_id=$comment_id");
     if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?id=$comment_id&message=db"); exit();}
   }
...
function show_pubs($parent_id)
{
	$objResponse = new xajaxResponse();
  $select_elements = '<select name="pub_id" style="width:280px;" size="6">';

  $result = mysql_query("select * from publications where type = 0 and parent_id = $parent_id order by order_id asc");
  if (mysql_num_rows($result) > 0)
   {
      while ($row = mysql_fetch_array($result))
       {
         $select_elements .= '<option value="'.$row['pub_id'].'">'.htmlspecialchars($row['head']).' (id: '.$row['pub_id'].')</option>';
       }
   }
...
$xajax->registerFunction("show_pubs");
 ...
 function show_select($parent_id = 0, $prefix = '')
  {
   ...
   $result = mysql_query("select *,
                          date_format(pub_comments.date, '%d.%m.%Y (%H:%i:%s)') as date2
                          from publications, pub_comments
                          where pub_comments.pub_id = publications.pub_id and
                          pub_comments.comment_id=$comment_id");
   if (!$result) exit();
...
if (isset($_GET['message']))
 ...
 echo '<h2>'.htmlspecialchars($row['head']).'</h2>';
...
echo'      </td></tr>
    ...
    <tr>
      ...
      <td>
         <select name="parent_id" style="width:280px;" onchange="xajax_show_pubs(this.form.parent_id.options[this.form.parent_id.selectedIndex].value);">
