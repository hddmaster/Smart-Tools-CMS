...
if (isset($_POST['content_name']))
 {
  ...
  if (use_field($content_name,'content','content_name','and type = 1')) {Header("Location: ".$_SERVER['PHP_SELF']."?message=duplicate");exit();}

  //Добавляем...
  $result = mysql_query("insert into content values (null,1,'$content_name','')");
...
// сортировка
 ...
 if (isset($_GET['query_str']) && trim($_GET['query_str']) !== '')
  {
    ...
    $add .= " and (content_name like '$query_str' or data like '$query_str')";
  }

 $query = "select * from content where type = 1 $add";
 ...
 echo '<tr align="center" class="header">
         ...
         <td nowrap width="35%">Модули CMS</td>
         <td nowrap width="35%">Страницы сайта</td>
         <td>&nbsp;</td>
       </tr>';
 ...
 while ($row = mysql_fetch_array($result))
  {
   echo '<tr valign="center" onmouseover="this.style.backgroundColor=\'#EFEFEF\'" onmouseout="this.style.backgroundColor=\'white\'" class="underline">
	   ...
	   <td class="grey">';
	     ...
	     $res = mysql_query("select * from auth_script_groups, auth_script_group_content where auth_script_groups.group_id = auth_script_group_content.group_id and auth_script_group_content.module_id = ".$row['obj_id']." order by group_name asc");
	     if (mysql_num_rows($res) > 0)
	      {
	        $i = 1;
	        while($r = mysql_fetch_array($res))
		 {
		   echo '<a class="grey" href="javascript:sw(\'/admin/editors/edit_auth_script_group.php?id='.$r['group_id'].'\');">'.htmlspecialchars($r['group_name']).'</a>';
