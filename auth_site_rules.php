 ...
 if ($user->check_user_rules('edit'))
  {
   ...
   $result = mysql_query("select script_id from auth_site_scripts");
         ...
         if (isset($_POST['access_'.$script_id]))
          {
            ...
            $res = mysql_query("select * from auth_site_rules where user_type=$user_type and script_id=$script_id");

            //пустые права
            if (mysql_num_rows($res) > 0 && $key == 0)
              mysql_query("delete from auth_site_rules where user_type=$user_type and script_id=$script_id");

            //обновляем права
            if (mysql_num_rows($res) > 0 && $key > 0)
              mysql_query("update auth_site_rules set access='$access' where user_type=$user_type and script_id=$script_id");

            //добавляем права
            if (mysql_num_rows($res) == 0 && $key > 0)
              mysql_query("insert into auth_site_rules values ($user_type,$script_id,'$access')");
          }
         else
             ...
             foreach ($types as $user_type => $user_type_name)
              {
                ...
                if (isset($_GET['user_type']) && $_GET['user_type']== $user_type) echo ' selected';
...
if (isset($_GET['user_type']) && $_GET['user_type']!='')
{
 ...
 $result = mysql_query("select * from auth_site_scripts order by script_id desc");
 if (@mysql_num_rows($result) > 0)
 {
 ...
 while ($row = mysql_fetch_array($result))
  {
   ...
   echo '<tr valign="center" onmouseover="this.style.backgroundColor=\'#EFEFEF\'" onmouseout="this.style.backgroundColor=\'white\'" class="underline">
           ...
           <td>'.basename($row['script_path']).' &nbsp; <span class="small">('.$row['script_path'].')</span></td>';

   $access = array(0, 0, 0, 0, 0);
   $res = mysql_query("select access from auth_site_rules where user_type=$user_type and script_id=$script_id");
