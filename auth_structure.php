...
if (isset($_POST['username']))
 {
  ...
  $query = "insert into auth values (null,
                                     $parent_id,
                                     0,
                                     1,
                                     '$username',
  ...
  if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?message=db"); exit();}
   ...
   // перенумеровываем
   $result = mysql_query("select * from auth where parent_id = $parent_id order by order_id asc");
  ...
  for($no=0; $no<count($items); $no++)
   {
     ...
     mysql_query("update auth set parent_id = ".$tokens[1].", order_id = ".$orders[$tokens[1]]." where user_id = ".$tokens[0]);
   }
...
function is_begin($user_id, $parent_id)
 {
   $result = mysql_query("select * from auth where parent_id = $parent_id and type != 2 order by order_id asc");
