...
if (isset($_POST['script_descr']) && isset($_POST['script_path']))
 {
 if ($user->check_user_rules('add'))
  {
   ...
   if (use_field($script_path,'auth_site_scripts','script_path')) {Header("Location: ".$_SERVER['PHP_SELF']."?message=duplicate");exit();}

   //Добавляем...
   $result = mysql_query("insert into auth_site_scripts values (null, '$script_descr', '$script_path')");
   ...
   if ($action == 'del')
    {
     if ($user->check_user_rules('delete'))
      {
     $result = mysql_query("delete from auth_site_scripts where script_id=$script_id");
      if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?message=db1"); exit();}

      $result = mysql_query("delete from auth_site_rules where script_id=$script_id");
      if (!$result) {Header("Location: ".$_SERVER['PHP_SELF']."?message=db2"); exit();}
 ...
 echo '<form action="" method="post">
   <table cellpadding="4" cellspacing="1" border="0" class="form">
    <tr>
      ...
      <td>
       <input style="width:280px" type="text" name="script_path" maxlength="255">
      </td>
    </tr>
    <tr>
      <td>Описание</td>
...
if (isset($_GET['query_str']) && trim($_GET['query_str']) !== '')
 ...
 $query = "select * from auth_site_scripts where 1 $add";
 ...
 while ($row = mysql_fetch_array($result))
  {
   echo '<tr valign="center" onmouseover="this.style.backgroundColor=\'#EFEFEF\'" onmouseout="this.style.backgroundColor=\'white\'" class="underline">
           ...
           <td align="center">'.$row['script_path'].'</td>
           <td align="center">'; if(!$row['script_descr']) echo '&nbsp;'; else echo htmlspecialchars($row['script_descr']); echo '</td>
           <td nowrap align="center">
           <a href="javascript:sw(\'/admin/editors/edit_auth_site_script.php?id='.$row['script_id'].'\');"><img align="absmiddle" src="/admin/images/icons/pencil.png" border="0" alt="Редактировать файл"></a>
           &nbsp;<a href="';
