...
class Auth {
    ...
    public function create_secret_key() {
        return base64_encode(serialize(array($this->username, md5($this->username.$_SERVER['HTTP_HOST'].$_SERVER['HTTP_USER_AGENT'].$this->control_str.$this->password))));
    }

    public function create_auth_pass_key() {
        return base64_encode(serialize(array($this->username, md5($this->username.$_SERVER['HTTP_HOST'].$this->control_str.$this->password))));
    }

    private function check_valid_session() {
        return (($this->check_value($_SESSION['valid_cms_user'])) ? true : false);
    }

    private function check_valid_cookie() {
        return (($this->check_value($_COOKIE['valid_cms_user'])) ? true : false);
    }

    public function check_value($value) {
        list ($username, $str) = unserialize(base64_decode($value));
        $result = mysql_query("select * from auth where username='$username' and status = 1");
    ...
    public function login($username, $password) {
        if (trim($username) && trim($password)) {
            ...
            $result = mysql_query("select * from auth where username='$username' and password='$password' and status = 1");
    ...
    private function check_session() {
        ...
        //создаем секретный ключ
        $key = $this->create_secret_key();
        $result = mysql_query("select * from auth_online where user_id = $this->user_id and hash = '$key'");
    ...
    private function save_session() {
        ...
        $key = $this->create_secret_key();
        $time = time();
        mysql_query("insert into auth_online values ($this->user_id, $time, '$key')");
    }

    public function count_session() {
        $result = mysql_query("select * from auth_online");
    ...
    private function clear_session() {
        ...
        $result = mysql_query("delete from auth_online where time < $expire_time");
    }

    public function check_valid_user() {
        //механизм сессий
        if (isset($_SESSION['valid_cms_user']) || isset($_COOKIE['valid_cms_user'])) {
            $key = false;
            //если установлен cookie, а сессия не активна
            if(!isset($_SESSION['valid_cms_user'])) {
                if ($this->check_valid_cookie()) {
                    $_SESSION['valid_cms_user'] = $_COOKIE['valid_cms_user'];
                    list ($username, $str) = unserialize(base64_decode($_SESSION['valid_cms_user']));
                    $result = mysql_query("select * from auth where username = '$username' and status = 1");
            ...
            if ($this->check_valid_session()) {
                ...
                list ($username, $str) = unserialize(base64_decode($_SESSION['valid_cms_user']));
                $result = mysql_query("select * from auth left join auth_users on auth.user_type = auth_users.user_type where username='$username' and status = 1");
                if (!$result) return false;
                if(mysql_num_rows($result) > 0) {
    ...
    public function get_user_rules() {
        $result = mysql_query(  "select
                                auth_rules.access,
                                auth_scripts.script_path
                                from auth_rules,auth_scripts
                                where
                                auth_scripts.script_id = auth_rules.script_id and
                                auth_rules.user_type = $this->user_type");
        if (mysql_num_rows($result) > 0) {
        ...
        if (array_key_exists($script, $this->user_rules)) {
            ...
            for ($i = 0; $i < 5; $i++) {
                ...
                if ($action == $this->actions[$i] &&
                    ...
                    if ($add_to_history && $action !== 'view') $this->history($action);
                    return true;
                }
            }
        }
    ...
    public function history($command = '', $other_data_array = array()) {
        ...
        mysql_query("insert into auth_history
                    (
                     `date`,
                     `user_id`,
                     `file`,
