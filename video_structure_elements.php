...
function save_video_structure($save_string)
{
  ...
  for($no=0; $no<count($items); $no++)
   {
     ...
     mysql_query("update video set parent_id = ".$tokens[1].", order_id = ".$orders[$tokens[1]]." where element_id = ".$tokens[0]);
   }
...
$xajax->registerFunction("save_video_structure");
//------------------------------------------------------------------------------
...
if ($user->check_user_rules('view','/admin/video_structure.php')) $tabs->add_tab('/admin/video_structure.php', 'Структура', 1);
if ($user->check_user_rules('view','/admin/video_comments.php')) $tabs->add_tab('/admin/video_comments.php', 'Комментарии');
$tabs->show_tabs();

$tabs2 = new Tabs;
$tabs2->level = 1;
if ($user->check_user_rules('view','/admin/video_structure.php')) $tabs2->add_tab('/admin/video_structure.php', 'Группы');
if ($user->check_user_rules('view','/admin/video_structure_elements.php')) $tabs2->add_tab('/admin/video_structure_elements.php', 'Публикации');
$tabs2->show_tabs();
      ...
      while ($row = mysql_fetch_array($result))
        {
          ...
          echo ' <span class="grey"> (id: '.$row['element_id'].')';
	  if ($row['img_path1']) echo ' '.$row['img_path1'];
	  echo '</span>';
...
function is_end($element_id, $parent_id)
 ...
 $parent_id = 0;
 $element_name = 'Корень галереи';
 if (isset($_GET['parent_id']) && $_GET['parent_id'] !== '0')
  {
    $parent_id = $_GET['parent_id'];
    $result = mysql_query("select * from video where element_id = $parent_id");
...
function save_tree()
 {
   ...
   xajax_save_video_structure(saveString);
 }
